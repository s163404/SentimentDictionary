...
def get_tweets_byquery(word:str, max_id):
    ...
    client = MongoClient('localhost', 27017)
    db = client.tweet_db
    collection = db.anonymous

    # レートリミット検証
    api, key_id = proc_ratelimit(api, key_id)

    # 1回目取得
    if max_id: results = api.search(word, lang='ja', count=100, max_id=str(max_id), result_type="recent")
    else: results = api.search(word, lang='ja', count=100, result_type="recent")
...
"""
def main():
    # get_tweets_byquery("\"iPhone 11\"|\"iPhone11\"", 1178046026990702600)
    # get_tweetobj("\"虫さん\"")
